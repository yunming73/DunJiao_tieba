  ...
  <div class="bas_item" v-if="bastext">
      <div class="container">
          <el-breadcrumb separator=">" style="margin:10px">
            ...
            <el-breadcrumb-item :to="{ path: `/bas/item/${bastext.id}` }">{{bastext.name}}贴吧</el-breadcrumb-item>
            ...
            <div class="bas_header_bottom">
                <div class="bas_header_bottom_left">
                    ...
                    <div class="bottom_left_img">
                        <RouterLink :to="`/bas/item/${bastext.id}`" class="bottom_left_img_a">
                ...
                <div class="bas_header_bottom_right" v-if="$store.state.user.profile.token">
                    <el-button type="danger" class="bottom_left_text_right" v-if="ba_show" @click="ba_botton(false)">
                            <el-icon :size="20" class="bottom_left_text_right_1"><Plus/></el-icon>
                            关注
                        </el-button>
                        <el-button type="info" class="bottom_left_text_right" v-else  @click="ba_botton(true)">
                            取消关注
                        </el-button>
                </div>
...
import {getContentsComment,basSearch,getBaEssence,addbaCollect,delbaCollect,searchContent,getLandlordContentComment} from '@/api/bas'
...
export default {
    ...
    setup(){
        ...
        if(store.state.user.profile.token){
            getBasCollect(userid).then(data=>{
                for(let i=0;i<data.data.length;i++){
                    if(data.data[i].id==route.params.id) return ba_show.value=false
                }
            })
        }
        // 吧信息
        ...
        // 主贴所属从帖列表
        const getlist=()=>{
            getContentsComment({query:route.params.commentid,pagenum:pagination.pagenum,pagesize:pagination.pagesize,sort:pagination.sort}).then(data=>{
        ...
        getBaEssence(route.params.id).then(data=>{
            if(data.meta.status!==200) return ElMessage.error('获取贴吧加精帖子出错')
            baEssence.value=data.data
        })
        // 楼主发言的从贴列表
        const getListLandlord=()=>{
            getLandlordContentComment({query:route.params.commentid,pagenum:pagination.pagenum,pagesize:pagination.pagesize,sort:pagination.sort,userid:texttitle.value.user_id}).then(data=>{
        ...
        const ba_botton=(val)=>{
            if(val){
                if(store.state.user.profile.token){
                    delbaCollect({baid:route.params.id,userid:userid}).then(data=>{
