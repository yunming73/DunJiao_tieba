                ...
                <div class="user_info">
                    ...
                    <div class="user_basic">
                        <div class="user_basic_top">
                            <!-- 名字 -->
                            <RouterLink to="/member/revise/1" class="user_basic_top_1">{{userinfo.nickname||userinfo.username}}</RouterLink>
                            <!-- 图标 -->
                    ...
                    <div class="user_button" v-if="$store.state.user.profile.token">
                        <el-button type="danger" size='large' v-if="user_show" @click="user_botton(false)">关注</el-button>
                        <el-button type="info" size='large' v-else @click="user_botton(true)">已关注</el-button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    setup(){
        ...
        if(store.state.user.profile.token){
            seaLike({memberid:user_id,userid:store.state.user.profile.id}).then(data=>{
        ...
        const user_botton=(val)=>{
            if(val){
                delLike({memberid:user_id,userid:store.state.user.profile.id}).then(data=>{
                    if(data.meta.status!==200) return ElMessage.error('取消关注失败')
                    ElMessage({message: '取消关注成功',type: 'success',})
                })
            }else{
                addLike({memberid:user_id,userid:store.state.user.profile.id}).then(data=>{
