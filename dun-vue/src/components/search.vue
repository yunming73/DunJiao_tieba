    ...
    <div class="container container_1">
          ...
          <div class="ban_botom_item" >
              <div v-loading="loadingcontent" :data="contents.result">
                <div class="ban_botom_item_for" v-if="contents.totalpage>0">
                  <div class="content" v-for="(item,i) in contents.result" :key="i">
      ...
      <div class="home_user"  v-if="user_states">
          <div class="user_info_content">
            ...
            <RouterLink to="/member" class="user_word">{{$store.state.user.profile.nickname||$store.state.user.profile.account}}</RouterLink>
            <!-- 个人信息 -->
          ...
          <div class="user_info_bottom" >
            ...
            <div class="user_info_bottom_ba2">
              <RouterLink class="user_info_bottom_item" v-for="(item,i) in basCollect" :key="i" :to="`/bas/item/${item.id}`"  :title="item.name">
...
export default {
 name:'search',
    ...
    setup(){
        ...
        const getlist=()=>{
          getBas({query:searchInfo.value, pagesize:pagination.pagesize, pagenum:pagination.pagenum }).then(data=>{
        ...
        const getlistTwo=()=>{
          searchContent({query:searchInfo.value, pagesize:paginations.pagesizes, pagenum:paginations.pagenums }).then(data=>{
            if(data.meta.status!==200) return ElMessage.error('模糊搜索帖子出错')
