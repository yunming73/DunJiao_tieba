  ...
  <el-card :body-style="{ padding: '10px' }" class="body_item" shadow="hover">
    ...
    <div class="item_left">
        <div class="item_left_left">
            ...
            <div class="item_title">
                <RouterLink :to="`/bas/item/${content.ba_id}/${content.id}`" @click="content_click()">
                    {{content.title}}
                </RouterLink>
            </div>
            <!-- 内容 -->
    ...
    <div class="item_right">
        ...
        <div class="item_right_top" v-if="source=='ba'">
            <RouterLink :to="content.co_user_id==$store.state.user.profile.id?`/member`:`/user/${content.co_user_id}`" :title="content.user_nickname||content.user_name">
        ...
        <div class="item_right_top" v-if="source=='shouye'">
            <RouterLink :to="`/bas/item/${content.ba_id}`" :title="content.ba_name">
       ...
       <div v-else-if="content.ba_team_ids && content.ba_team_ids.includes($store.state.user.profile.id)">
            <!-- 删除按钮 -->
            <div class="other_1">
                <el-popconfirm
                    confirmButtonText='好的'
       ...
       <div v-else>
            <!-- 删除按钮 -->
            <div class="other_1" v-if="content.co_user_id==$store.state.user.profile.id" >
                <el-popconfirm
                    confirmButtonText='好的'
                    cancelButtonText='不用了'
                    icon="el-icon-info"
...
export default {
    ...
    setup(props){
        ...
        const content_click=()=>{
            if(store.state.user.profile.token){
                addHistoryRecord({ userid:store.state.user.profile.id, contentid:props.content.id }).then(data=>{
                    if(data.meta.status!==200) return ElMessage.error('添加浏览记录失败')
                })
            }
        }
        return{content_click,handleDeleteRow,show,handlEessenceAdd,coEssence,handlEessenceDel,contentStats,like_show_2,like_show_3,ba_botton,ba_botton_two}
    }
}
</script>
