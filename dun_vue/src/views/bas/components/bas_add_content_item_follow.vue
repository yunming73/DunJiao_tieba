...
import {addFollowComment} from '@/api/bas';
import {uploadContent} from '@/api/bas'
import {useStore} from 'vuex'
import {ElMessage} from 'element-plus'
import { useRoute } from 'vue-router';
export default {
    name:'basaddcontentitemfollow',
    setup(){
        ...
        const rules = reactive({
            itemcontent: [
                ...
                { max: 300, message: '长度最大300', trigger: 'change' },
            ],
        })
        // 发表
        ...
        await formEl.validate((valid, fields) => {
            if (valid) {
                const  {itemcontent }=ruleForm
                const userid=store.state.user.profile.id
                const baid=route.params.id
                const commentid=route.params.commentid
                addFollowComment({ content:itemcontent, fatherid:commentid, userid:userid, postid:baid, img:imgs.value }).then(data=>{
                    if(data.meta.status!==200) return ElMessage.error('发表从贴失败')
                    ElMessage({type: 'success',message: '发表从贴成功'})
                    imgs.value=[]
                    formEl.resetFields()
                    location.reload()
                })
            } else {
                ElMessage.error('从贴错误提交')
            }
        })
        }
        // 重置
